 /**
 Nowadays gyms are essential for everyone, to combats diseases, keep in good shape or even to lift the mood and improve sleeping! And we all can see its effect on our society since it’s an unneglectable part of everyone’s routine.
This gym management system was designed using object-oriented programming by applying all its concepts!
...
public class GymManagmentSystem_442003697 {
    ...
    //setting phone numbers to the trainers    
        ...
        switch(in){
           ...
           case 3: isTrainer();break;
           
           case 4 : Exit(false); break loop ;
    ...
    /**EditingData instance method that Editing stored member data
    *Edit the userâ€™s data by using the saved ID number in the ArrayList allow the user to change the email when choosing the second case, and change the phone number when choosing the third case.He can renew the subscription when choosing the fourth case, and when choosing the fifth case, the memberâ€™s membership will be deleted and he will be removed from the Arraylist
    */
       ...
       boolean Existing = false;
        ...
        //-----------------------------------------------------------------------------------      
              ...
              {   
                  ...
                  case 1:  members.get(index).setMyPackages( (  Packages )GoldenPackage.clone());
                             members.get(index).subscriptionEnddate(choicce);break;
                             
                  case 2:  members.get(index).setMyPackages( (  Packages )PopularPackage.clone());
                        members.get(index).subscriptionEnddate(choicce);break;
                        
                        
                  case 3:   members.get(index).setMyPackages( (  Packages )StandardPackage.clone());
                            members.get(index).subscriptionEnddate(choicce);break;
...
//              System.out.println(ex.getMessage());
    ...
    /**
      * NewMember instance method that adding the new members to the gym by creating object from class member then receive the required
      * information from the user and validate it in setter methods and by using try-catches then after the user entered correct information
      * our program will display menu of all available packages that the member can choice it by calling static method menu and 
      * at the end our program will save the id of the member in the array list of id that for the members and 
      * will add the new member object to the array list that of type members 
        ...
        try{     
             ...
             System.out.print("Please enter your choice of these packages that you want to get");
              ...
              {   
                  ...
                  case 1: m.setMyPackages( (  Packages )GoldenPackage.clone());
                          m.subscriptionEnddate(choicce);break;
                          
                  case 2:m.setMyPackages( (  Packages )PopularPackage.clone());
                       m.subscriptionEnddate(choicce);break;
                       
                  case 3:m.setMyPackages( (  Packages )StandardPackage.clone());
                       m.subscriptionEnddate(choicce);break;
     ...
     * 
     * @param t this method from the main we send true or false when we want to terminate the loop in the main method to 
     * terminate all the program we send false
     */
    ...
    /**
     * static method menu it display the information of all the packages in our gym to the user to make it chose 
     * one from it to subscribe to it 
     */
    ...
    /**
       ...
       * Method isTrainer updates the trainer's data after making sure that he is already a trainer in gym
         by entering his ID number 
       * If the ID number already exists, several options will appear for her
        including displaying the workouts she has today
        and updating his number phone or email, 
        but if If the ID is outside the specified range
        the program will print a message to the trainer that we could not find the number entered
       
       * We ask the trainer about her desire to display the worhouts she has today
        If her answer is yes the workouts
        she is doing today will appear to her according to the ID number she entered previously
        But if the answer is no, another message will appear, and if her answer is not yes or no, 
        then it will have many attempts until a yes or no answer is entered We used the while loop
      
        * Then we ask the trainer if she wants to update her information
        * If her answer is yes the The options will appear If she chooses the number 1 or number 2 
         and here used switch - case 
        it will enable her to update her phone number We used  
        in this option parseInt method from wrapper class and NumberFormatException and object from
        Trainer class to update the number phone in setNumberPhone
        
        *If she chooses the number 2, the second option appears for her, which is to update her email 
         and used if - else and object from Trainer class, check from enterd string if has "@gmail.com"
         update new Email in setEmail 
         else another massage will appear 
         
         *If the trainer enters the wrong phone number or the wrong email 
         we give her attempts to enter the correct number by using two diffrent loops
 ...
 and we put the ID numbers in a certain range from 100 to 108
  If the ID does not exist in this field, he cannot use the services      
      ...
      and thus show him the exercises he has today    
                   ...
                   boolean test = false;//this variable to test if we have to iterate again to ask the user about its choice 
       ...
       or 2 to update his email
            ...
            if it is less or entered incorrectly, the else message will appear to the trainer
                                                ...
                                                update.setNumberPhone(newNumberPhone);
                                                System.out.println("the new number phone is : " + update.numberPhone);
                                                ask = false;
         ...
         a specific format for the email that the program will accept
                                ...
                                case 2:
                                    ...
                                    do {
                                        ...
                                        if (newEmail.indexOf(x) + x.length() == newEmail.length()) {
                                            ...
                                            System.out.println("the new Email is ; " + update.email);
                                            continue1 = false;
                                        } else {
                                ...
                                default:
                                    ...
                                    test=true;
                                    System.out.println("this options is unavailable , Please try again ");
                            }//end of object 
                    }while(test==true);//loop iterate until the user inter corret chice
                            break;
                        ...
                        System.out.println("what do you mean by " + change + " please anwser \"yes\" or \"no\" ");
            ...
            System.out.println("couldn't find your ID");
        }
